...
*/		
		...
		var testResults = '';
		var testResultsHeader = '<html><head><style type="text/css">body{font-size:14px;font-family:helvetica;background-color:#000000;color:#ffffff;}</style></head><body>';
    ...
    TitaniumReporter.prototype = {
			...
			if (!spec.results().passed()) {
				...
				for (var i=0; i<spec.results().items_.length; i++) {
					...
					if (!spec.results().items_[i].passed_) {
						this.log('&nbsp;&nbsp;&nbsp;&nbsp;(' + (i+1) + ') ' + spec.results().items_[i].message + '<br>');
        ...
        reportSuiteResults: function(suite) {
            ...
            this.log('<span style="color:#00ff00;"><b>[' + suite.description + ']</span> '
            	+'<span style="color:#eeeeee">'            	
            	+results.passedCount + ' of ' + results.totalCount
            	+' assertions passed.</span></b><br><br>'
            );
        },
